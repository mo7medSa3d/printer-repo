...
suite("production-like PostgreSQL migration upgrade", () => {
  ...
  beforeAll(async () => {
    ...
    const oldJournal = { ...journal, entries: oldEntries };
    await writeFile(join(oldDir, "meta", "_journal.json"), JSON.stringify(oldJournal));
  ...
  it("applies current migration 0017 over an existing populated 0016 database without data loss", async () => {
    ...
    try {
      ...
      await pool.query(`INSERT INTO agents (id, branch_id, secret, status, lifecycle) VALUES ($1, $2, $3, 'online', 'active')`, [agentId, branchId, "upgrade-secret-hash"]);
      ...
      const before = await pool.query(`SELECT b.id AS branch_id, a.id AS agent_id, p.id AS printer_id, j.id AS job_id, j.idempotency_key FROM branches b JOIN agents a ON a.branch_id = b.id JOIN printers p ON p.agent_id = a.id JOIN print_jobs j ON j.printer_id = p.id WHERE b.id = $1`, [branchId]);
      expect(before.rowCount).toBe(1);
      expect(before.rows[0].idempotency_key).toBe("legacy-upgrade-key");

      await migrate(db, { migrationsFolder: currentDir });

      const after = await pool.query(`SELECT b.id AS branch_id, a.id AS agent_id, p.id AS printer_id, j.id AS job_id, j.idempotency_key FROM branches b JOIN agents a ON a.branch_id = b.id JOIN printers p ON p.agent_id = a.id JOIN print_jobs j ON j.printer_id = p.id WHERE b.id = $1`, [branchId]);
