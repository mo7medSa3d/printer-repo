...
export async function applyMigrations(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const check = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name = 'print_jobs' AND column_name IN ('delivered_at','acked_at','delivery_attempts')`);
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      try {
        ...
        const res = await client.query(`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename IN ('agents','api_keys','auth_rate_limits','branches','destinations','discovered_devices','discovery_sessions','document_types','local_networks','manager_sessions','printer_bindings','printers','print_jobs')`);
        const existing = (res.rows as { tablename: string }[]).map((r) => r.tablename).sort();
        if (existing.length > 0) await client.query(`TRUNCATE TABLE ${existing.join(", ")} RESTART IDENTITY CASCADE`);
...
export async function seedFixture(opts?: { branchId?: string; printerCapabilities?: unknown }): Promise<Fixture> {
  ...
  try {
    ...
    try {
      ...
      const hasBranchCol = await client.query(`SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='printers' AND column_name='branch_id' LIMIT 1`);
...
export async function insertQueuedJob(f: Fixture, jobId: string, opts?: { expiresInMs?: number }): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await client.query("COMMIT");
    } catch (e) { try { await client.query("ROLLBACK"); } catch {} throw e; } finally { await client.query("SELECT pg_advisory_unlock($1)", [GLOBAL_PG_LOCK]); }
  } finally { client.release(); }
}
