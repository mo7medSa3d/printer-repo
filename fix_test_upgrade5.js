...
file = file.replace('const oldEntries = journal.entries.slice(0, 38);', 'const oldEntries = journal.entries.slice(0, 17);');
file = file.replace('if (i < 38) await cp(join("drizzle", migrations[i]), join(oldDir, migrations[i]));', 'if (i < 17) await cp(join("drizzle", migrations[i]), join(oldDir, migrations[i]));');
file = file.replace('const target = i < 38 ? oldDir : currentDir;', 'const target = i < 17 ? oldDir : currentDir;');
file = file.replace('if (i < 38) await cp(join("drizzle", migrations[i]), join(currentDir, migrations[i]));', 'if (i < 17) await cp(join("drizzle", migrations[i]), join(currentDir, migrations[i]));');
