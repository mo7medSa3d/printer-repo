...
This stack runs the Next.js gateway and PostgreSQL together. The Agent connects to the gateway over HTTP/WebSocket; the Windows Agent is still installed on the client PC and is not containerized here.
...
The gateway container waits for PostgreSQL to become healthy, applies all Drizzle migrations, and then starts the custom Next.js server. The custom server also hosts the Agent WebSocket.
...
PostgreSQL data is stored in the named Docker volume `postgres_data`. Recreating the containers does not delete the database.
...
The reverse proxy must support WebSocket upgrade for `/api/agent/ws`.
...
3. Apply the gateway migrations.
4. Start the gateway and verify `/api/health`.
5. Connect one Windows Agent.
6. Configure a test printer and branch/binding.
7. Run a test print.
8. Verify job creation, claiming, delivery, status transitions, and retry/stale recovery.
