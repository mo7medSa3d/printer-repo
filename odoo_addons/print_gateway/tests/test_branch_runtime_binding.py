...
class TestBranchRuntimeBinding(TransactionCase):
    def setUp(self):
        ...
        with patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None):
            ...
            values = {"gateway_url": "https://gateway.example.com", "gateway_api_key": "test-key", "enabled": True}
            if self.config:
                self.config.write(values)
            else:
                self.config = config_model.create({"company_id": self.company.id, **values})

    def _gateway_gets(self):
    ...
    def _binding_values(self, **extra):
        values = {
    ...
    def test_retired_agent_is_rejected(self):
        with patch("odoo.addons.print_gateway.models.binding.requests.get", return_value=Response({"agents": self.agents})), patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None), self.assertRaises(ValidationError):
            self.env["print_gateway.binding"].create(self._binding_values(runtime_agent_id="agent-old"))
    ...
    def test_printer_from_another_agent_is_rejected(self):
        ...
        with patch("odoo.addons.print_gateway.models.binding.requests.get", side_effect=self._gateway_gets()), patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None), self.assertRaises(ValidationError):
            record._validate_runtime_target()

    def test_destination_from_parent_company_is_rejected_for_branch(self):
        ...
        values = self._binding_values(destination_type="picking_type", destination_picking_type_id=picking_type.id, report_id=report.id)
        with self.assertRaises(ValidationError):
            self.env["print_gateway.binding"].create(values)

    def test_valid_full_binding_persists_and_creates_branch_assignment(self):
        with patch("odoo.addons.print_gateway.models.binding.requests.get", side_effect=self._gateway_gets()), patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None):
            binding = self.env["print_gateway.binding"].create(self._binding_values())
    ...
    def test_persisted_binding_reopens_with_same_context(self):
        with patch("odoo.addons.print_gateway.models.binding.requests.get", side_effect=self._gateway_gets()), patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None):
            binding = self.env["print_gateway.binding"].create(self._binding_values(priority=20))
    ...
    def test_find_for_prefers_branch_binding(self):
        with patch("odoo.addons.print_gateway.models.binding.requests.get", side_effect=self._gateway_gets()), patch("odoo.addons.print_gateway.models.gateway_config.PrintGatewayConfig._validate_gateway_host", return_value=None):
            binding = self.env["print_gateway.binding"].create(self._binding_values(priority=30))
    ...
    def test_agent_change_clears_printer(self):
        record = self.env["print_gateway.binding"].new(self._binding_values())
