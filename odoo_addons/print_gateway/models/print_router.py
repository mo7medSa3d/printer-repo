...
class PrintGatewayRouter(models.AbstractModel):
    ...
    def destination_for(self, *, report=None, record=None, explicit_destination=None):
        return self.env["print_gateway.binding"].destination_for(record=record, report=report, explicit_destination=explicit_destination)
    ...
    def _submit_route(self, *, route, payload, company, report=None, source_model=None, source_record_id=None, idempotency_key=None):
        ...
        return {"gateway_enabled": True, "native": False, "status": job.status, "job_id": job.id, "message": _("Print job %s accepted by the Gateway.") % job.id}
