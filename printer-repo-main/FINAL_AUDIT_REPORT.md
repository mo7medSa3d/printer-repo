...
against it with byte-level/path-level `diff -qr` checks. 83 repository files were changed or
...
## CI follow-up: GitHub Actions failure `fix #45`

A post-delivery GitHub Actions run exposed six test failures in three regression files. Those failures were reviewed against the actual failing stack traces and corrected in the repository delivered with this follow-up.

Fixed CI failures:

1. `tests/regression-critical.test.ts` was asserting that the phrase `exactly-once physical` must never appear. The architecture intentionally documents the non-guarantee, so the test was corrected to reject positive claims such as `exactly-once physical printing is guaranteed/supported`, not the required disclaimer itself.
2. `tests/architecture-hardening.test.ts` expected the canonical printer schema to reject legacy `type`, but the Zod object stripped unknown keys. The canonical schema is now `.strict()`; legacy normalization remains confined to `normalizeLegacyPrinterInput()`.
3. `tests/phase1-branch-authorization.test.ts` mocked printers without mocking their authoritative Agent owner. The fixture now models `Printer -> Agent -> Branch`, matching the production ownership contract.
4. `tests/regression-critical.test.ts` expected obsolete Odoo wording (`requires spooler or IPP`). The expectation now matches the current explicit PDF/raw safety contract.
5. The dashboard no longer carries a derived `branchId` field on the Printer view model; it derives the displayed Branch directly through the authoritative Agent relation.

The GitHub Actions run that reported the failures had `11` test files passed, `3` failed and `141` tests passed, `6` failed, with `60` skipped. The failure stack traces matched the issues above.
