...
function DiscoveryPanel({ agents, branchesById }: { agents: Agent[]; branchesById: Map<string, Branch> }) {
  ...
  return (
    <div className="space-y-4">
          ...
          {activeAgents.length === 0 ? (
            ...
            activeAgents.map((a) => (
              <option key={a.id} value={a.id}>
                {a.name} — {branchesById.get(a.branchId)?.name ?? a.branchId}
              </option>
            ))
          )}
        </Select>
      ...
      {msg && (
        <div className="rounded-lg border border-edge bg-surface-2 px-3.5 py-2 text-xs text-ink-2 font-medium">
          {msg}
        </div>
      )}
      ...
      {devices.length === 0 ? (
        <p className="text-xs text-ink-3">
          No candidates yet — select an active agent and start a scan. Scans private /24 subnets only; does not impact active printing.
        </p>
      ) : (
        <div className="space-y-2.5 max-h-[28rem] overflow-auto pr-1">
          {devices.map((d: any) => (
            <div key={d.id} className="rounded-xl border border-edge bg-surface p-3.5 text-xs space-y-1.5 shadow-xs">
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold text-ink text-sm truncate">
                  {d.deviceName ?? d.model ?? d.ipAddress ?? d.id}
                </span>
                <StatusBadge
                  tone={d.confidence === "high" ? "ok" : d.confidence === "medium" ? "warn" : "neutral"}
                  label={d.confidence ?? "low confidence"}
                />
              </div>
              <div className="text-ink-3">
                Protocol: <span className="font-mono text-ink-2">{d.protocol}</span> · {d.ipAddress ? `${d.ipAddress}:${d.port ?? ""}` : d.uri ?? ""}
              </div>
              <div className="text-ink-3">
                Verification: {d.verification === "verified" ? "Verified" : "Candidate"} · Status: {d.candidateStatus ?? "discovered"}
              </div>
              {d.manufacturer || d.model ? (
                <div className="text-ink-2 font-medium">{d.manufacturer ?? ""} {d.model ?? ""}</div>
              ) : null}
              <div className="mt-2.5 flex items-center justify-between gap-2 border-t border-edge pt-2.5">
                <Mono>{d.id.slice(0, 16)}…</Mono>
                <Button
                  size="sm"
                  variant={d.candidateStatus === "provisioned" ? "secondary" : "primary"}
                  onClick={() => provision(d.id)}
                  disabled={busy || d.candidateStatus === "provisioned"}
                >
...
export default function DashboardClient({
  ...
  const handleTestConnection = async (printerId: string) => {
    ...
    try {
      ...
      if (data.error) {
        setNotice({ text: `Reachable: ${data.reachable} · Agent Online: ${data.agentOnline} · ${data.error}`, type: "error" });
      } else {
        setNotice({ text: `Printer reachable! Agent online · Round-trip latency: ${data.latencyMs ?? "active"}ms`, type: "success" });
      }
    } catch (err) {
  ...
  const filteredPrinters = useMemo(() => {
    ...
    if (printerStatusFilter !== "all") {
      list = list.filter((p) => p.status === printerStatusFilter);
    }
    return list;
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="px-6 pb-6 space-y-4">
            ...
            <Field label="Register agent">
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Select
                  ...
                  disabled={isLoading || databaseError !== null}
                  className="sm:w-40"
                >
                  <option value="">Branch…</option>
                  {initialBranches.filter(b => b.enabled).map(b => (
                ...
                <Button
                  variant="primary"
                  type="submit"
                  disabled={isLoading || !newAgentName || !newAgentBranchId || databaseError !== null}
          ...
          {initialAgents.length === 0 ? (
            <EmptyState
              ...
              description="Create an agent to generate a pairing code, then pair your desktop manager or Windows agent."
            />
          ) : (
            <div className="space-y-3 max-h-[36rem] overflow-auto pr-1">
              {initialAgents.map((agent) => (
                <div key={agent.id} className="card card-interactive p-4 shadow-none">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="truncate font-semibold text-ink text-sm">{agent.name}</h3>
                  ...
                  <div className="mt-2.5 space-y-1.5 text-xs text-ink-2">
                    ...
                    {agent.pairingCode ? (
                      <div className="flex items-center gap-1.5 rounded-lg border border-warn-edge bg-warn-bg px-3 py-1.5">
                        ...
                        <span className="text-ink-2 font-medium">Pairing code</span>
                    ...
                    <p className="text-ink-3">
                      Branch: {branchesById.get(agent.branchId)?.name ?? agent.branchId} · Printers: {agent.printerCount}
                    </p>
                    <p className="text-ink-3">
                  ...
                  {agent.lifecycle !== "retired" && (
                    <div className="mt-3 flex justify-end gap-2 border-t border-edge pt-2.5">
                      {agent.lifecycle === "active" && (
                        <Button variant="ghost" size="sm" onClick={() => setPendingLifecycle({ agent, lifecycle: "disabled" })}>
                          Disable
                        </Button>
                      )}
                      {agent.lifecycle === "disabled" && (
                        <Button variant="ghost" size="sm" onClick={async () => { await setAgentLifecycle(agent.id, "active"); window.location.reload(); }}>
                          Enable
                        </Button>
                      )}
                      <Button variant="ghost" size="sm" onClick={() => setPendingLifecycle({ agent, lifecycle: "retired" })}>
                        Retire
                      </Button>
                    </div>
                  )}
        ...
        <div className="px-6 pb-6 space-y-3">
          ...
          {filteredPrinters.length === 0 ? (
            <EmptyState
              ...
              title={initialPrinters.length === 0 ? "No printers yet" : "No matching printers"}
              description={initialPrinters.length === 0 ? "Printers appear here as soon as an online agent reports them." : "Try adjusting your search criteria."}
            />
          ) : (
            <div className="space-y-3 max-h-[36rem] overflow-auto pr-1">
              {filteredPrinters.map((printer) => (
                <div key={printer.id} className="card card-interactive p-4 shadow-none">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="truncate font-semibold text-ink text-sm">{printer.name}</h3>
                    <StatusBadge tone={printerTone(printer.status)} label={printer.status.charAt(0).toUpperCase() + printer.status.slice(1)} />
                  </div>
                  <div className="mt-2 space-y-1.5 text-xs text-ink-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="uppercase font-mono text-[11px]">{printer.connectionType} · {printer.printerType}</span>
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <Button variant="secondary" size="sm" onClick={() => handleTestConnection(printer.id)} disabled={isLoading || printer.lifecycle !== "active"}>
                      Test Probe
                    </Button>
                    <Button variant="primary" size="sm" onClick={() => handleTestPrint(printer.id)} disabled={isLoading || printer.lifecycle !== "active"}>
                      Test Print
                    </Button>
                  </div>
                  {printer.lifecycle !== "retired" && (
                    <div className="mt-2.5 flex justify-end gap-2 border-t border-edge pt-2">
                      {printer.lifecycle === "active" && (
                        <Button variant="ghost" size="sm" onClick={() => setPendingLifecycle({ printer, lifecycle: "disabled" })}>
                          Disable
                        </Button>
                      )}
                      {printer.lifecycle === "disabled" && (
                        <Button variant="ghost" size="sm" onClick={async () => { await setPrinterLifecycle(printer.id, "active"); window.location.reload(); }}>
                          Enable
                        </Button>
                      )}
                      <Button variant="ghost" size="sm" onClick={() => setPendingLifecycle({ printer, lifecycle: "retired" })}>
                        Retire
                      </Button>
                    </div>
                  )}
      ...
      <Card className="overflow-hidden lg:col-span-1">
        <CardHeader
          ...
          subtitle="Scan local network for physical printers"
          icon={<Network className="h-5 w-5 text-brand" aria-hidden />}
        />
      ...
      <Card className="overflow-hidden lg:col-span-3">
        <CardHeader
          title="Recent Print Queue"
          subtitle="Latest jobs routed through the cloud gateway"
          icon={<Clock className="h-5 w-5 text-brand" aria-hidden />}
          actions={
            <div className="w-48 sm:w-64">
              <Input
                placeholder="Search queue by job ID..."
                value={jobSearch}
        ...
        <div className="px-6 pb-6 space-y-2">
          {filteredJobs.length === 0 ? (
            <EmptyState icon={<Clock className="h-8 w-8" />} title="No jobs in queue" description="Jobs submitted via API or Odoo will appear here." />
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredJobs.map((job) => (
                    <tr key={job.id} className="border-b border-edge last:border-0 row-hover">
                      <td className="px-4 py-3.5 font-mono text-xs">
                        <Mono>{job.id}</Mono>
                      </td>
                      <td className="px-4 py-3.5 text-xs text-ink-2 font-medium">
                        {initialPrinters.find((p) => p.id === job.printerId)?.name || job.printerId}
                      </td>
                      <td className="px-4 py-3.5">
                        <StatusBadge
                          tone={jobTone(job.status)}
                          label={job.status === "success" ? "Completed" : job.status.charAt(0).toUpperCase() + job.status.slice(1)}
                        />
                      </td>
                      <td className="px-4 py-3.5 text-right text-xs text-ink-3">
      ...
      <Modal
        ...
        footer={
          <>
            ...
            >
              {pendingLifecycle?.lifecycle === "retired" ? "Retire" : "Disable"}
            </Button>
          </>
        }
      >
        <p className="text-sm text-ink-2">
          Note: Retirement is permanent and cannot be reversed.
        </p>
      </Modal>
