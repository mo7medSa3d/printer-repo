...
export default async function DashboardPage() {
  ...
  try {
    ...
    const printerRows = await db.select({ printer: printers, branchId: agents.branchId }).from(printers).innerJoin(agents, eq(agents.id, printers.agentId)).orderBy(desc(printers.createdAt));
    allPrinters = printerRows.map(({ printer, branchId }) => ({ ...printer, branchId }));
