...
export async function sweepPrintJobs(scope: { agentId?: string } = {}): Promise<{ expired: number; requeuedClaims: number; stalePrinting: number; exhaustedClaims: number }> {
  ...
  const expired = await db.execute(sql`
    UPDATE print_jobs SET status='expired',
      error=CASE WHEN status='printing' THEN 'JOB_EXPIRED_DURING_PRINT: physical output is unknown (full, partial or none)' ELSE NULL END,
      updated_at=now()
  ...
  const requeuedClaims = await db.execute(sql`
    UPDATE print_jobs SET status='queued', claimed_at=NULL, delivered_at=NULL, acked_at=NULL,
      retries=retries+1, updated_at=now()
    WHERE status='claimed' AND updated_at < now() - make_interval(secs => ${STALE_CLAIM_SECONDS})
