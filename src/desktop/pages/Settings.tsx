        ...
        >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="primary" onClick={s.startAgent} icon={<Play className="h-[18px] w-[18px]" />}>
                Start
              </Button>
              <Button variant="secondary" onClick={s.requestStopAgent} icon={<Square className="h-[18px] w-[18px]" />}>
                Stop
              </Button>
              <Button variant="ghost" onClick={s.restartAgent} icon={<RotateCcw className="h-[18px] w-[18px]" />}>
                Restart
              </Button>
            </div>
          </div>
          ...
          <div className="section-rule pt-5">
            <div className="flex items-start justify-between gap-4">
              ...
              >
                <span className={`inline-block h-5 w-5 transform rounded-full bg-surface shadow-xs transition-transform ${s.autostart ? "translate-x-6" : "translate-x-1"}`} />
              </button>
            </div>
          </div>
        </SettingsSection>
        ...
        <div className="grid gap-6 px-6 py-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <ol className="space-y-3 text-[14px] text-ink-2">
            {[
              ...
              "Generate a six-digit pairing code on the gateway dashboard.",
              "Enter the six digits here — the code is one-time and expires after 30 minutes.",
            ].map((step, i) => (
          ...
          <div className="flex w-full max-w-sm items-end gap-3">
            <Field label="Pairing code" htmlFor="pair-code" hint="6 digits">
              <Input
                id="pair-code"
                type="text"
                inputMode="numeric"
                pattern="[0-9]{6}"
                value={s.pairCode}
                onChange={(e) => s.setPairCode(e.target.value.replace(/\D/g, "").slice(0, 6))}
                placeholder="123456"
                maxLength={6}
            ...
            <Button variant="primary" onClick={s.pair} loading={s.busy} icon={<ShieldCheck className="h-[18px] w-[18px]" />}>
              Pair agent
            </Button>
          </div>
        </div>
          ...
          <div className="grid gap-7 border-t border-edge px-6 py-6 lg:grid-cols-2">
            <div>
              ...
              <p className="text-[13px] leading-relaxed text-ink-2">
                Pairing uses a one-time six-digit code; invalid attempts are rate-limited per source and credentials are stored locally with OS-level protection.
              </p>
                ...
                <div className="space-y-2">
                  {paths.map(([label, path]) => (
                    <div key={label} className="flex items-center gap-2 rounded-lg border border-edge bg-surface-2 px-3.5 py-2.5">
                      ...
                      <CopyButton value={path} label="Copy" onCopied={() => s.setMsg({ text: "Copied to clipboard", type: "success" })} />
                    </div>
                  ))}
                </div>
              ) : (
