...
The Agent has bounded, best-effort discovery detectors for configuration/registry, Windows
spooler, USB, RAW TCP, IPP/IPPS, LPR, SNMP, WSD and mDNS sources. These sources are additive and
results are deduplicated before they are reported to the Gateway.

Discovery results intentionally distinguish **candidate** from **verified** devices. Verification
and confidence are heuristic/device-dependent; discovery is not a guarantee that every printer
model, vendor protocol or network topology will be detected. Windows-only capabilities such as
native USB and spooler enumeration are unavailable on non-Windows hosts.

The orchestrator enforces a 30-second upper bound for a discovery session and reports a failed
or cancelled session when that bound is exceeded. Individual network detectors also use their
own shorter bounded timeouts.
...
PostgreSQL-backed suites require `DATABASE_URL` and are a required CI gate. Odoo, Windows,
network-printer and USB-printer E2E are environment-dependent and are not represented as
passing unless that environment actually ran.
...
3. `POST /api/printers/:id/test-connection` returns cached heartbeat data; there is no live
   probe.
4. **Discovery is best-effort.** mDNS, SNMP and WSD are implemented as bounded detectors, but
   protocol/vendor coverage is not certified across every device family and environment.
