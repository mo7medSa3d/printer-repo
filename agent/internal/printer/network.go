...
func (p *NetworkPrinter) Print(ctx context.Context, data []byte) error {
	...
	if len(data) > maxPrintBytes { return fmt.Errorf("payload %d bytes exceeds %d limit", len(data), maxPrintBytes) }
	d := net.Dialer{Timeout: dialTimeout}
	conn, err := d.DialContext(ctx, "tcp", p.Address)
	if err != nil { return fmt.Errorf("dial %s: %w", p.Address, err) }
	defer conn.Close()

	written := 0
	for written < len(data) {
		select { case <-ctx.Done(): return fmt.Errorf("print cancelled after %d/%d bytes: %w", written, len(data), ctx.Err()); default: }
		_ = conn.SetWriteDeadline(time.Now().Add(writeStallTimeout))
		n, err := conn.Write(data[written:])
		written += n
		if err != nil { return fmt.Errorf("write %d/%d to %s: %w", written, len(data), p.Address, err) }
		if n == 0 { return fmt.Errorf("short write 0 bytes to %s", p.Address) }
	}
	return nil
}
...
func (p *NetworkPrinter) PrintDocument(ctx context.Context, doc Document) error {
	...
	case KindImage:
		escpos, err := JPEGToESCPOS(doc.Data)
		if err != nil { return fmt.Errorf("convert POS image for raw printer: %w", err) }
		return p.Print(ctx, escpos)
	default:
