...
// dedupeKey intentionally uses identity material before endpoint material.
...
func probeSNMPHost(ctx context.Context, host string, timeout time.Duration) *DeviceInfo {
	...
	caps := addVerification(map[string]interface{}{
		"discovered_via": "snmp",
		"discovery_protocol": "snmp",
		"sysDescr": sysDescr,
	}, "candidate", "medium", SourceSNMP)
	if parts := strings.Fields(sysDescr); len(parts) >= 2 {
		caps["manufacturer"] = parts[0]
	}
	return &DeviceInfo{
		ID:             StableIDFromNetwork(host, 9100),
...
func buildSNMPGet(oids []string) []byte {
	...
	body := bytes.Buffer{}
	body.Write([]byte{0x02, 0x01, 0x00})
...
func encodeOID(s string) []byte {
	...
	for i, part := range parts {
		if _, err := fmt.Sscanf(part, "%d", &values[i]); err != nil || values[i] < 0 {
			return nil
		}
	}
...
func extractSNMPString(data []byte) string {
	...
	for i := 0; i+len(targetOID) < len(data); i++ {
		...
		length, next, ok := readBERLength(data, pos+1)
		if !ok || length == 0 || length > 512 || next+length > len(data) {
			continue
		}
...
func probeLPRHost(ctx context.Context, host string, timeout time.Duration) *DeviceInfo {
	...
	buf := make([]byte, 256)
	n, err := conn.Read(buf)
	if err != nil || n == 0 || buf[0] != 0x00 {
		return nil
	}
	caps := addVerification(map[string]interface{}{"discovered_via": "lpr", "discovery_protocol": "lpr", "lpr_verified": true, "queue": "raw"}, "verified", "high", SourceLPR)
	return &DeviceInfo{
...
func discoverWSDPrinters(ctx context.Context) []DeviceInfo {
	...
	for time.Now().Before(deadline) && ctx.Err() == nil {
		...
		if !strings.Contains(lower, "print") && !strings.Contains(lower, "printer") && !strings.Contains(lower, "wprt") {
			continue
		}
		ip := remote.IP.String()
		if seenIP[ip] {
		...
		manufacturer := extractXMLTag(string(data), "wsdp:Manufacturer")
		if manufacturer == "" {
			manufacturer = extractXMLTag(string(data), "Manufacturer")
		}
		caps := addVerification(map[string]interface{}{"discovered_via": "wsd", "discovery_protocol": "wsd"}, "candidate", "medium", SourceWSD)
		if manufacturer != "" {
...
func buildWSDProbe() []byte {
	...
	id[6] = (id[6] & 0x0f) | 0x40
	id[8] = (id[8] & 0x3f) | 0x80
	uuid := fmt.Sprintf("urn:uuid:%08x-%04x-%04x-%04x-%012x", binary.BigEndian.Uint32(id[0:4]), binary.BigEndian.Uint16(id[4:6]), binary.BigEndian.Uint16(id[6:8]), binary.BigEndian.Uint16(id[8:10]), uint64(id[10])<<40|uint64(id[11])<<32|uint64(id[12])<<24|uint64(id[13])<<16|uint64(id[14])<<8|uint64(id[15]))
...
<soap:Body><wsd:Probe><wsd:Types>wprt:PrintDeviceType</wsd:Types></wsd:Probe></soap:Body></soap:Envelope>`, uuid))
}
...
func extractXMLTag(s, tag string) string {
	...
	end := strings.Index(s[valueStart:], "</"+tag+">")
	if end < 0 {
		return ""
	}
	return strings.TrimSpace(s[valueStart : valueStart+end])
}

func discoverFullMDNS(ctx context.Context) []DeviceInfo {
	services := []string{"_ipp._tcp.local", "_ipps._tcp.local", "_printer._tcp.local"}
	...
	for _, service := range services {
		if query := buildMDNSQueryReal(service); query != nil {
	...
	for time.Now().Before(deadline) && ctx.Err() == nil {
		...
		for _, host := range parseMDNSHosts(buf[:n]) {
			...
			caps := addVerification(map[string]interface{}{"discovered_via": "mdns", "discovery_protocol": "mdns"}, "candidate", "medium", SourceMDNS)
			if host.Model != "" {
				caps["model"] = host.Model
			}
			if host.Manufacturer != "" {
				caps["manufacturer"] = host.Manufacturer
			}
			if host.UUID != "" {
				caps["uuid"] = host.UUID
			}
			name := host.Name
			if name == "" {
				name = fmt.Sprintf("mDNS Printer %s", host.IP)
			}
			out = append(out, DeviceInfo{
				ID:             StableIDFromNetwork(host.IP, port),
				Name:           name,
				DisplayName:    name,
				PrinterType:    "unknown",
				ConnectionType: "ipp",
				Protocol:       "ipp",
				Endpoint:       fmt.Sprintf("ipp://%s:%d/ipp/print", host.IP, port),
...
func parseMDNSHosts(data []byte) []mdnsHost {
	// This compatibility parser only supplies candidates. Full DNS-SD
	// verification is handled by the dedicated IPP/mDNS verifier path.
	s := string(data)
	model := ""
	if idx := strings.Index(strings.ToLower(s), "product="); idx >= 0 {
		...
		model = strings.TrimSpace(s[start : start+end])
	}
	uuid := ""
	if idx := strings.Index(strings.ToLower(s), "uuid="); idx >= 0 {
		start := idx + len("uuid=")
		end := strings.IndexByte(s[start:], 0)
		if end < 0 {
			end = 64
		...
		uuid = strings.TrimSpace(s[start : start+end])
	}
	for i := 0; i+6 <= len(data); i++ {
		if data[i] != 0x00 || data[i+1] != 0x04 {
			continue
		}
		ip := net.IPv4(data[i+2], data[i+3], data[i+4], data[i+5])
		if ip.IsPrivate() && !ip.IsLoopback() {
			return []mdnsHost{{IP: ip.String(), Model: model, UUID: uuid}}
		}
	}
	return nil
}
